...
class BallTracker:
    ...
    def detect_frame(self,frame):
        results = self.model.predict(frame, conf=0.2)[0] # One object only

        ball_dict = {}  # Key: ball_id, Output: bounding box
        # There is only one object in my ball detect model(yolo5_last.pt)
    ...
    def draw_bboxes(self,video_frames, player_detections):
        output_video_frames = []
        for frame, ball_dict in zip(video_frames, player_detections): # Loop same time
            # Draw Bounding Boxes
